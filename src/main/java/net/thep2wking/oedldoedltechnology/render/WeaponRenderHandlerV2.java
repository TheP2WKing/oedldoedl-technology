...
public class WeaponRenderHandlerV2 {
   ...
   public void onHandRender(RenderSpecificHandEvent event) {
      ...
      if (event.getHand() == EnumHand.MAIN_HAND && !weapon.isEmpty() && weapon.getItem() instanceof EnergyWeapon) {
         ...
         float f = 0.07F;
         Project.gluPerspective(35.0F, (float)this.mc.displayWidth / (float)this.mc.displayHeight, 0.05F, (float)(this.mc.gameSettings.renderDistanceChunks * 32));
         ...
         if (this.mc.gameSettings.thirdPersonView == 0 && !flag && !this.mc.gameSettings.hideGUI && !this.mc.playerController.isSpectator()) {
            float zoomValue = Sine.easeInOut(ClientWeaponHandler.ZOOM_TIME, 0.0F, 1.0F, 1.0F);
            float recoilValue = Quad.easeInOut(ClientWeaponHandler.RECOIL_TIME, 0.0F, 1.0F, 1.0F) * MathHelper.clamp(ClientWeaponHandler.RECOIL_AMOUNT, 0.0F, 20.0F);
   ...
   public void renderModel(IBakedModel model, ItemStack weapon) {
      if (model != null) {
         ...
         this.renderQuads(worldrenderer, model.getQuads((IBlockState)null, (EnumFacing)null, 0L), WeaponHelper.getColor(weapon), weapon);
         tessellator.draw();
      }
   }
   ...
   private void setupViewBobbing(float partialTicks) {
      if (this.mc.getRenderViewEntity() instanceof EntityPlayer) {
         ...
         GlStateManager.translate(MathHelper.sin(f1 * 3.1415927F) * f2 * 0.05F, -Math.abs(MathHelper.cos(f1 * 3.1415927F) * f2) * 0.1F, 0.0F);
