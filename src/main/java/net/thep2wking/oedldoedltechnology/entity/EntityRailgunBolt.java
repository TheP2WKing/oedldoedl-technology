...
public class EntityRailgunBolt extends Entity implements IProjectile, IGravityEntity, IEntityAdditionalSpawnData {
	...
	private boolean manageExplosions(RayTraceResult hit) {
		if (explodeMultiply > 0) {
			if (!world.isRemote) {
				world.newExplosion(null, hit.hitVec.x, hit.hitVec.y, hit.hitVec.z, explodeMultiply, false, true);
			} else {
