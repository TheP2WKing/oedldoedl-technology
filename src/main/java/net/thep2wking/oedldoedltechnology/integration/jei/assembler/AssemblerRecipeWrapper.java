package net.thep2wking.oedldoedltechnology.integration.jei.assembler;
...
public class AssemblerRecipeWrapper implements IRecipeWrapper {
	...
	public static List<AssemblerRecipeWrapper> getRecipes() {
		List<AssemblerRecipeWrapper> recipes = new ArrayList<>();
		for (AssemblerRecipe recipeAssembler : AssemblerRecipeRegistry.getRecipeList()) {
			recipes.add(new AssemblerRecipeWrapper(recipeAssembler.getInput1(), recipeAssembler.getInput2(),
					recipeAssembler.getOutput1(), recipeAssembler.getTime()));
		}
		return recipes;
	}
}