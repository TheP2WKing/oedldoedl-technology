...
public class ModPowerSlugGen implements IWorldGenerator {
    ...
    private void generatePowerSlugs(World world, Random rand, int x, int z) {
        if (!TechnologyConfig.WORLD.POWER_SLUG) {
            return;
        }

        int attempts = (int) (TechnologyConfig.WORLD.POWER_SLUG_CHANCE * 10);
        for (int i = 0; i < attempts; i++) {
            ...
            if (state.getBlock() instanceof BlockLog) {
                ...
                if (world.isAirBlock(offsetPos)) {
                    ...
                    switch (color) {
                        case BLUE:
                            slugState = ModBlocks.BLUE_POWER_SLUG.getDefaultState().withProperty(BlockPowerSlug.FACING,
                                    facing);
                            break;
                        case YELLOW:
                            slugState = ModBlocks.YELLOW_POWER_SLUG.getDefaultState()
                                    .withProperty(BlockPowerSlug.FACING, facing);
                            break;
                        case PURPLE:
                            slugState = ModBlocks.PURPLE_POWER_SLUG.getDefaultState()
                                    .withProperty(BlockPowerSlug.FACING, facing);
                            break;
                        default:
                            slugState = ModBlocks.BLUE_POWER_SLUG.getDefaultState().withProperty(BlockPowerSlug.FACING,
